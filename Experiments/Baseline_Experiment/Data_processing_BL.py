...
def compute_comparative_metrics(base_save_dir: str):
    ...
    true_minimas_y = minima_data["y"] # Shape: (3, n_repeats, 1)
    ...
    n_dims, n_methods, n_repeats, N_iters, n_fns = y_true_store.shape
    ...
    for k in tqdm(range(n_dims), desc="Computing Metrics per Dimension"):

        # Loop through all reapeat runs
        for rep in range(n_repeats):

            # Get minimum value for current function
            y_opt_continuous = true_minimas_y[k, rep, 0].item()
            ...
            for t in range(N_iters):
                ...
                for m_idx in range(n_methods):
                    ...
                    y_best_found = y_best_store[k, m_idx, rep, t, 0].item()
                    metrics["regret"][k, m_idx, rep, t] = abs(y_opt_continuous - y_best_found)
                    
                    # Prediction error of surrogate
                    if m_idx < 2:
                        queried_y = y_true_store[k, m_idx, rep, t, 0].item()
                        pred_mu = mu_store[k, m_idx, rep, t, 0].item()
                        metrics["pred_error"][k, m_idx, rep, t] = abs(pred_mu - queried_y)
                
                # Rank between methods
                y_t_vals = y_true_store[k, :, rep, t, 0].numpy()
                # Negate y_t_vals so the highest objective gets rank 1
                ranks = rankdata(-y_t_vals, method='min') 
                for m_idx in range(n_methods):
                    metrics["rank_of_method"][k, m_idx, rep, t] = ranks[m_idx]
        
        # Inter-Method Spearman Correlation (Across repeats)
        for t in range(N_iters):
            gp_vals = y_best_store[k, 0, :, t, 0].numpy()
            pfn_vals = y_best_store[k, 1, :, t, 0].numpy()
            rand_vals = y_best_store[k, 2, :, t, 0].numpy()
            ...
            metrics["spearman_gp_pfn"][k, t] = rho_gp_pfn if not np.isnan(rho_gp_pfn) else 0.0
            metrics["spearman_gp_rand"][k, t] = rho_gp_rand if not np.isnan(rho_gp_rand) else 0.0
            metrics["spearman_pfn_rand"][k, t] = rho_pfn_rand if not np.isnan(rho_pfn_rand) else 0.0
...
if __name__ == "__main__":
    ...
    base_dir = "results_BL_150/run_20260727_005330" 
