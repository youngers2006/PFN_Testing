...
def main():
    ...
    # Initialise storage
    maxima_store_y = torch.zeros((3, n_repeats, 1), dtype=torch.float64, device='cpu')
    maxima_store_x = [torch.zeros((n_repeats, dim), dtype=torch.float64, device='cpu') for dim in dims]

    # Filepath
    base_save_dir = "results_BL_150/run_20260727_005330"
    
    for k in range(3):
        dim = dims[k]
        
        for rep in tqdm(range(n_repeats), desc=f"On dimension {dim}"):
            filepath = f"results_BL_150/run_20260727_005330/dim_{dim}/repeat_{rep}/problem.npz"
