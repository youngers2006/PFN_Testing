...
class RFFSampler:
    ...
    def __init__(self, 
                 ...
                 lengthscales: Optional[List] = None, 
        ...
        ls_tensor = torch.tensor(lengthscales)

        if self.kernel == "RBF":
            # RBF Kernel
            self.omegas = torch.as_tensor(np.random.normal(size=(self.num_features, self.input_dim))) / ls_tensor
        elif self.kernel == "Matern52":
            # Matern 5/2 Kernel
            self.omegas = torch.as_tensor(multivariate_t.rvs(df=5, size=(self.num_features, self.input_dim))) / ls_tensor
        elif self.kernel == "Matern32":
            # Matern 3/2 Kernel
            self.omegas = torch.as_tensor(multivariate_t.rvs(df=3, size=(self.num_features, self.input_dim))) / ls_tensor
        elif self.kernel == "Matern12":
            # Matern 1/2 Kernel
            self.omegas = torch.as_tensor(multivariate_t.rvs(df=1, size=(self.num_features, self.input_dim))) / ls_tensor
