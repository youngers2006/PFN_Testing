...
from Experiments.BO_run_Experiments.Irrelevant_Features_Experiment.RFF import RFFSampler
...
def main():
    ...
    # Initialise storage
    maxima_store_y = [torch.zeros((n_repeats, 1), dtype=torch.float64, device='cpu') for _ in dims]
    maxima_store_x = [torch.zeros((n_repeats, dim), dtype=torch.float64, device='cpu') for dim in dims]

    # Filepath
    base_save_dir = "results_IF_150/run_20260728_162916"

    for k in range(3):
        dim = dims_real[k]
        
        for rep in tqdm(range(n_repeats), desc=f"on dimension {dim}", leave=False):
            #filepath = f"results_KT_50/run_20260714_231826/dim_{dim}/repeat_{rep}/problem.npz"
            filepath = f"results_IF_150/run_20260728_162916/dim_{dim}/repeat_{rep}/problem.npz"
            max_x, max_y = compute_true_rff_maximum(filepath)

            maxima_store_y[k][rep, :] = max_y
            max_x_tensor = torch.tensor(max_x)
            maxima_store_x[k][rep, :] = max_x_tensor
